...
/**
 * Helper function to get the base URL for Openrouter API
 *
 * @param runtime The runtime context
...
function getOpenAiApiKey(runtime: any): string | undefined {
  return getSetting(runtime, 'OPENAI_EMBEDDING_API_KEY');
}
...
function getLargeModel(runtime: any): string {
  return (
    getSetting(runtime, 'OPENROUTER_LARGE_MODEL') ?? getSetting(runtime, 'LARGE_MODEL', 'gpt-4o')
  );
}

/**
 * Create an Openrouter client with proper configuration
 *
 * @param runtime The runtime context
...
async function tokenizeText(model: ModelTypeName, prompt: string) {
  const modelName =
    model === ModelType.TEXT_SMALL
      ? (process.env.OPENROUTER_SMALL_MODEL ?? process.env.SMALL_MODEL ?? 'gpt-4o-mini')
...
async function detokenizeText(model: ModelTypeName, tokens: number[]) {
  const modelName =
    model === ModelType.TEXT_SMALL
      ? (process.env.OPENROUTER_SMALL_MODEL ?? process.env.SMALL_MODEL ?? 'gpt-4o-mini')
      : (process.env.OPENROUTER_LARGE_MODEL ?? process.env.LARGE_MODEL ?? 'gpt-4o');
  const encoding = encodingForModel(modelName as TiktokenModel);
...
async function fetchTextToSpeech(runtime: AgentRuntime, text: string) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Openrouter TTS error ${res.status}: ${err}`);
    }

    return res.body;
  } catch (err: any) {
    throw new Error(`Failed to fetch speech from Openrouter TTS: ${err.message || err}`);
  }
}
  ...
  async init(_config, runtime) {
    try {
      ...
      try {
        ...
        logger.warn(`Error validating Openrouter API key: ${fetchError}`);
        logger.warn('Openrouter functionality will be limited until a valid API key is provided');
        // Continue execution instead of throwing
      }
    } catch (error) {
    ...
    [ModelType.TEXT_EMBEDDING]: async (
      ...
      try {
        ...
        const response = await fetch(`${baseURL}/embeddings`, {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${getOpenAiApiKey(runtime)}`,
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({
        ...
        if (!response.ok) {
          logger.error(`OpenAI embedding API error: ${response.status} - ${response.statusText}`);
  ...
  tests: [
    {
      ...
      tests: [
        {
          name: 'openrouter_test_url_and_api_key_validation',
          fn: async (runtime) => {
            const baseURL = getBaseURL(runtime);
            const response = await fetch(`${baseURL}/models`, {
              headers: {
            ...
            if (!response.ok) {
              throw new Error(`Failed to validate Openrouter API key: ${response.statusText}`);
            }
          },
        },
        {
        ...
        // {
        //   name: 'openrouter_test_image_generation',
        //   fn: async (runtime) => {
        //     logger.log('openrouter_test_image_generation');
        //     try {
        ...
        //   fn: async (runtime) => {
        //     try {
        //       logger.log('openrouter_test_image_description');
        //       try {
        ...
        //       }
        //     } catch (e) {
        //       logger.error('Error in openrouter_test_image_description:', e);
        //     }
        //   },
        // },
        // {
        //   name: 'openrouter_test_transcription',
        //   fn: async (runtime) => {
        //     logger.log('openrouter_test_transcription');
        //     try {
        ...
        // },
        {
          name: 'openrouter_test_text_tokenizer_encode',
          fn: async (runtime) => {
        ...
        {
          name: 'openrouter_test_text_tokenizer_decode',
          fn: async (runtime) => {
        ...
        // {
        //   name: 'openrouter_test_text_to_speech',
        ...
        //     } catch (error) {
        //       logger.error('Error in openrouter_test_text_to_speech:', error);
        //       throw error;
        //     }
        //   },
        // },
