...
import { FilterOptions, Tweet } from './types';

class TweetFilter {
  private options: Partial<FilterOptions>;

  constructor() {
    this.options = {};
  }

  async promptCollectionMode(): Promise<FilterOptions> {
    const { mode } = await inquirer.prompt<{ mode: 'all' | 'custom' }>([
      {
        type: 'list',
        name: 'mode',
    ...
    if (mode === 'all') {
      ...
      Logger.info(`Selected Content Types: ${this.options.contentTypes.join(', ')}`);
  ...
  async promptCustomOptions(): Promise<FilterOptions> {
    Logger.info('Configure Custom Tweet Collection');

    const answers = await inquirer.prompt<FilterOptions>([
      {
        type: 'checkbox',
        name: 'tweetTypes',
        ...
        validate: (input) => input.length > 0 || 'Please select at least one tweet type.',
      },
      {
        type: 'checkbox',
        name: 'contentTypes',
        ...
        validate: (input) => input.length > 0 || 'Please select at least one content type.',
      },
      {
        type: 'confirm',
      ...
      },
      {
        ...
        message: 'Would you like to exclude tweets containing specific keywords?',
        default: false,
      },
      {
        type: 'input',
    ...
    if (answers.filterByEngagement) {
      configTable.push(['Min. Likes', answers.minLikes], ['Min. Retweets', answers.minRetweets]);
    }

    if (answers.filterByDate) {
      configTable.push(['Date Range', `${answers.startDate} to ${answers.endDate}`]);
    }

    if (answers.excludeKeywords) {
      configTable.push(['Excluded Keywords', answers.keywordsToExclude.join(', ')]);
    }
  ...
  shouldIncludeTweet(tweet: Tweet): boolean {
    if (
    ...
    if (!this.options.contentTypes?.includes('text') && !hasImage && !hasVideo && !hasLinks)
      return false;

    if (this.options.filterByEngagement) {
      if (tweet.likes < (this.options.minLikes || 0)) return false;
      if (tweet.retweetCount < (this.options.minRetweets || 0)) return false;
    }

    if (this.options.filterByDate && this.options.startDate && this.options.endDate) {
