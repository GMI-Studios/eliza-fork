...
import { Tweet, Analytics } from './types';

class DataOrganizer {
  private baseDir: string;

  constructor(baseDir: string, username: string) {
    this.baseDir = path.join(baseDir, username.toLowerCase(), format(new Date(), 'yyyy-MM-dd'));
   ...
   */
  getPaths(): Record<string, Record<string, string>> {
    return {
      raw: {
  ...
  async saveTweets(tweets: Tweet[]): Promise<Analytics> {
    ...
    try {
      // Save raw tweets
      await fs.writeFile(paths.raw.tweets, JSON.stringify(tweets, null, 2), 'utf-8');
      ...
      await fs.writeFile(paths.analytics.stats, JSON.stringify(analytics, null, 2), 'utf-8');
      ...
      Logger.info(`ℹ️  Generating fine-tuning data with ${finetuningData.length} entries`);
  ...
  generateAnalytics(tweets: Tweet[]): Analytics {
    ...
    const validDates = validTweets.map((t) => t.timestamp).sort((a, b) => a - b);
    ...
    return {
      ...
      engagement: {
        totalLikes: tweetsForEngagement.reduce((sum, t) => sum + (t.likes || 0), 0),
        totalRetweetCount: tweetsForEngagement.reduce((sum, t) => sum + (t.retweetCount || 0), 0),
        totalReplies: tweetsForEngagement.reduce((sum, t) => sum + (t.replies || 0), 0),
        averageLikes: (
      ...
      timeRange: {
        start: validDates.length ? format(new Date(validDates[0]), 'yyyy-MM-dd') : 'N/A',
      ...
      contentTypes: {
        ...
        textOnly: tweets.filter((t) => !t.photos?.length && !t.videos?.length && !t.urls?.length)
          .length,
      },
    };
  }
   ...
   */
  generateFinetuningData(tweets: Tweet[]): Array<{ text: string }> {
    return tweets
      .filter((tweet) => !tweet.isRetweet && tweet.text && tweet.timestamp !== null)
      .map((tweet) => {
   ...
   */
  generateSummary(tweets: Tweet[], analytics: Analytics): string {
