...
class TwitterPipeline {
  constructor(username) {
    ...
    this.paths.cookies = path.join(process.cwd(), "cookies", `${process.env.TWITTER_USERNAME}_cookies.json`)
  ...
  async collectWithFallback(searchQuery) {
    ...
    const fallbackTask = async ({ page }) => {
      ...
      try {
        ...
        while (unchangedCount < 3 && Date.now() - sessionStartTime < this.config.fallback.sessionDuration) {
          await page.evaluate(() => {
              ...
              .map((tweet) => {
                try {
                  return {
                    ...
                    metrics: Array.from(tweet.querySelectorAll('span[data-testid$="count"]')).map((m) => m.textContent),
                  }
                } catch (e) {
                  return null
                }
  ...
  async collectTweets(scraper) {
    try {
      ...
      Logger.info(`📊 Found ${chalk.bold(totalExpectedTweets.toLocaleString())} total tweets for @${this.username}`)
      ...
      try {
        const searchResults = scraper.searchTweets(`from:${this.username}`, this.config.twitter.maxTweets, SearchMode.Latest)
          ...
          if (tweet && !allTweets.has(tweet.id)) {
            ...
            if (processedTweet) {
              ...
              if (allTweets.size % 100 === 0) {
                ...
                Logger.info(`📊 Progress: ${allTweets.size.toLocaleString()} unique tweets (${completion}%)`)
  ...
  async run() {
    ...
    try {
      ...
      const successRate = ((allTweets.length / (this.stats.requestCount + this.stats.fallbackCount)) * 100).toFixed(1)
      ...
      console.log(chalk.cyan(`• With Images: ${analytics.contentTypes.withImages.toLocaleString()}`))
      console.log(chalk.cyan(`• With Videos: ${analytics.contentTypes.withVideos.toLocaleString()}`))
      ...
      console.log(chalk.cyan(`• Total Retweets: ${analytics.engagement.totalRetweetCount.toLocaleString()}`))
      console.log(chalk.cyan(`• Total Replies: ${analytics.engagement.totalReplies.toLocaleString()}`))
      console.log(chalk.cyan(`• Average Likes: ${analytics.engagement.averageLikes}`))

      // Collection method breakdown
      if (this.stats.fallbackUsed) {
        Logger.info("\n🔄 Collection Method Breakdown:")
        console.log(chalk.cyan(`• Primary Collection: ${(allTweets.length - this.stats.fallbackCount).toLocaleString()}`))
        console.log(chalk.cyan(`• Fallback Collection: ${this.stats.fallbackCount.toLocaleString()}`))
      }
  ...
  async logError(error, context = {}) {
    ...
    const errorLogPath = path.join(this.dataOrganizer.baseDir, "meta", "error_log.json")
