...
import { SearchMode, searchProfiles, searchTweets, searchQuotedTweets } from './search';
...
export class Client {
   ...
   */
  public async fetchHomeTimeline(count: number, seenTweetIds: string[]): Promise<Tweet[]> {
    if (!this.auth) {
   ...
   */
  public async login(bearerToken: string, refreshToken: string): Promise<void> {
    ...
    this.auth = new TwitterAuth(bearerToken, refreshToken);
  }
   ...
   */
  public async getDirectMessageConversations(userId: string, cursor?: string): Promise<any> {
   ...
   * @returns The response from the Twitter API
   */
  public async sendDirectMessage(conversationId: string, text: string): Promise<any> {
