  ...
  async processMentionTweets(mentionCandidates: ClientTweet[]) {
    ...
    for (const tweet of uniqueTweetCandidates) {
      if (!this.client.lastCheckedTweetId || BigInt(tweet.id) > this.client.lastCheckedTweetId) {
        ...
        const validTimestamp = this.client.validateAndFixTimestamp(tweet.timestamp);
        ...
        logger.debug(
          'Creating memory for interaction mention tweet',
          tweet.id,
          'validated timestamp:',
          validTimestamp
        );
  ...
  async buildConversationThread(tweet: ClientTweet, maxReplies = 10): Promise<ClientTweet[]> {
    ...
    async function processThread(currentTweet: ClientTweet, depth = 0) {
      ...
      if (!memory) {
        ...
        // Fix timestamp validation - ensure it's in milliseconds and valid
        const validTimestamp = this.client.validateAndFixTimestamp(currentTweet.timestamp);
